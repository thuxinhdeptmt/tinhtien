...
function renderPeople() {
  ...
  PEOPLE.forEach(person => {
    ...
    // Khay thêm loại khoản: mặc định chỉ hiện 1 dòng. Bấm vào mới xổ các lựa chọn.
    // Khay vẫn được căn xuống cùng cao độ với hai cột còn lại.
    if (inactiveGroups.length) {
      const tray = document.createElement("div");
      tray.className = "type-tray";
      tray.dataset.person = person;

      const wrap = document.createElement("div");
      wrap.className = "type-adder-wrap";
      ...
      toggle.addEventListener("click", () => {
        ...
        document.querySelectorAll(".type-menu").forEach(other => {
          if (other !== menu) other.hidden = true;
        });
        document.querySelectorAll(".type-adder-toggle").forEach(other => {
