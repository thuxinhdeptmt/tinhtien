...
function renderPeople() {
  ...
  PEOPLE.forEach(person => {
    ...
    panel.querySelector(".person-title").textContent = `${person} đã chi`;
    ...
    const inactiveGroups = groups.filter(group => !isGroupActive(group));

    activeGroups.forEach(group => activeContainer.appendChild(renderGroup(person, group)));

    const addTypeBtn = panel.querySelector(".add-type-btn");
    const typeMenu = panel.querySelector(".type-menu");

    if (inactiveGroups.length === 0) {
      addTypeBtn.hidden = true;
    } else {
      ...
      inactiveGroups.forEach(group => {
        ...
        btn.addEventListener("click", () => {
          state.editing = { person, groupId: group.id, rowIndex: group.rows.length - 1, isNew: true };
          typeMenu.hidden = true;
          activeContainer.appendChild(renderGroup(person, group, { forceVisible: true }));
...
function renderGroup(person, group, options = {}) {
  ...
  if (!options.forceVisible && !isGroupActive(group)) node.hidden = true;
  return node;
}
...
function renderGroupSummary(groupNode, group) {
  ...
  const main = groupNode.querySelector(".summary-main");
  const sub = groupNode.querySelector(".summary-sub");

  main.textContent = total > 0 ? formatMoney(total) : "";
  if (total <= 0) {
    sub.textContent = "";
  } else if (group.kind === "advance") {
    sub.textContent = `→ ${group.target} ${formatMoney(total)}`;
  } else {
    sub.textContent = `÷ ${formatMoney(share)}`;
  }
}
...
document.addEventListener("click", e => {
  document.querySelectorAll(".type-menu").forEach(menu => {
    if (!menu.closest(".type-adder-wrap")?.contains(e.target)) menu.hidden = true;
  });
});
